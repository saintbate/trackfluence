...
export async function GET(req: NextRequest) {
  ...
  const parsed = QuerySchema.safeParse({
    ...
    igUserId: url.searchParams.get("igUserId") || IG_DEFAULT_BUSINESS_ID,
  ...
  const { brandId, igUserId, from, to, dryRun } = parsed.data;
  ...
  let savedMedia = 0;
  if (dryRun === "0") {
    savedDaily = await upsertAccountDaily(brandId, igUserId || IG_DEFAULT_BUSINESS_ID || "", daily);
    savedMedia = await upsertMediaRows(brandId, igUserId || IG_DEFAULT_BUSINESS_ID || "", media);
  }
